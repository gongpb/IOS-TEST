...
#define  VideoPlayerStatus  @"status"
#define  VideoPlayerPlaybackBufferEmpty  @"playbackBufferEmpty"
#define  VideoPlayerStatusplaybackLikelyToKeepUp  @"playbackLikelyToKeepUp"
#define  playerBackgroundColor [UIColor blackColor].CGColor
...
{
    //检测URL
    if(![self checkAvailableWithURL:url]){
        return;
    }
    if(!superView){
        return;
    ...
    self.currentPlayerLayer.frame = CGRectMake(0, 0, superView.bounds.size.width, superView.bounds.size.height/2);
...
{
    //播放状态监听
    [_currentPlayerItem addObserver:self forKeyPath:VideoPlayerStatus options:NSKeyValueObservingOptionNew context:nil];
    
    [_currentPlayerItem addObserver:self forKeyPath:VideoPlayerPlaybackBufferEmpty options:NSKeyValueObservingOptionNew context:nil];
    [_currentPlayerItem addObserver:self forKeyPath:VideoPlayerStatusplaybackLikelyToKeepUp options:NSKeyValueObservingOptionNew context:nil];
}

//移除播放器播放监听
- (void)removePlayerObserver
{
    [_currentPlayerItem removeObserver:self forKeyPath:VideoPlayerStatus];
    [_currentPlayerItem removeObserver:self forKeyPath:VideoPlayerPlaybackBufferEmpty];
    [_currentPlayerItem removeObserver:self forKeyPath:VideoPlayerStatusplaybackLikelyToKeepUp];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
    ...
    else if ([keyPath isEqualToString:VideoPlayerPlaybackBufferEmpty])
    {
        [self processObserveBuffering];
    }
    else if([keyPath isEqualToString:VideoPlayerStatusplaybackLikelyToKeepUp])
    {
...
- (void)setCurrentPlayerLayer:(AVPlayerLayer *)currentPlayerLayer
{
    ...
    _currentPlayerLayer.backgroundColor = playerBackgroundColor;
